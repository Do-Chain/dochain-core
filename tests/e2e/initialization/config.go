...
var (
	...
	StakeAmountCoinA = sdk.NewCoin(DoDenom, StakeAmountIntA)
	StakeAmountIntB  = sdkmath.NewInt(StakeAmountB)
	StakeAmountCoinB = sdk.NewCoin(DoDenom, StakeAmountIntB)

	InitBalanceStrA = fmt.Sprintf("%d%s", DoBalanceA, DoDenom)
...
func updateMintGenesis(mintGenState *minttypes.GenesisState) {
	mintGenState.Params.MintDenom = DoDenom
}

func updateBankGenesis(bankGenState *banktypes.GenesisState) {
	denomsToRegister := []string{DoDenom, AtomDenom}
...
func updateTreasuryGenesis(treasuryGenState *treasurytypes.GenesisState) {
	...
	treasuryGenState.Params.TaxPolicy = treasurytypes.PolicyConstraints{
		...
		Cap:     sdk.NewCoin(DoDenom, sdkmath.NewInt(100000000000000)),
	}
}
...
func updateGovGenesis(govGenState *govv1.GenesisState) {
	...
	govGenState.Params.MinDeposit = tenDo
}

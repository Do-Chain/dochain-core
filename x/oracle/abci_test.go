...
func TestOracleThreshold(t *testing.T) {
	...
	prevoteMsg := types.NewMsgAggregateDoRatePrevote(hash, keeper.Addrs[0], keeper.ValAddrs[0])
	voteMsg := types.NewMsgAggregateDoRateVote(salt, exchangeRateStr, keeper.Addrs[0], keeper.ValAddrs[0])

	_, err1 := h.AggregateDoRatePrevote(sdk.WrapSDKContext(input.Ctx.WithBlockHeight(0)), prevoteMsg)
	_, err2 := h.AggregateDoRateVote(sdk.WrapSDKContext(input.Ctx.WithBlockHeight(1)), voteMsg)
	require.NoError(t, err1)
	require.NoError(t, err2)

	oracle.EndBlocker(input.Ctx.WithBlockHeight(1), input.OracleKeeper)

	_, err := input.OracleKeeper.GetDoExchangeRate(input.Ctx.WithBlockHeight(1), core.MicroSDRDenom)
	require.Error(t, err)
	...
	prevoteMsg = types.NewMsgAggregateDoRatePrevote(hash, keeper.Addrs[1], keeper.ValAddrs[1])
	voteMsg = types.NewMsgAggregateDoRateVote(salt, exchangeRateStr, keeper.Addrs[1], keeper.ValAddrs[1])
	...
	prevoteMsg = types.NewMsgAggregateDoRatePrevote(hash, keeper.Addrs[2], keeper.ValAddrs[2])
	voteMsg = types.NewMsgAggregateDoRateVote(salt, exchangeRateStr, keeper.Addrs[2], keeper.ValAddrs[2])
	...
	rate, err := input.OracleKeeper.GetDoExchangeRate(input.Ctx.WithBlockHeight(1), core.MicroSDRDenom)
...
func TestOracleDrop(t *testing.T) {
	...
	input.OracleKeeper.SetDoExchangeRate(input.Ctx, core.MicroKRWDenom, randomExchangeRate)
...
func TestOracleTally(t *testing.T) {
	...
	for i, rate := range rates {
		...
		prevoteMsg := types.NewMsgAggregateDoRatePrevote(hash, sdk.AccAddress(valAddrs[i]), valAddrs[i])
		voteMsg := types.NewMsgAggregateDoRateVote(salt, exchangeRateStr, sdk.AccAddress(valAddrs[i]), valAddrs[i])
...
func TestOracleTallyTiming(t *testing.T) {
	...
	oracle.EndBlocker(input.Ctx, input.OracleKeeper)
	_, err := input.OracleKeeper.GetDoExchangeRate(input.Ctx, core.MicroSDRDenom)
	require.Error(t, err)
...
func TestOracleRewardDistribution(t *testing.T) {
	...
	err := input.BankKeeper.MintCoins(input.Ctx, types.ModuleName, sdk.NewCoins(sdk.NewCoin(core.MicroDoDenom, rewardsAmt)))
	require.NoError(t, err)
	...
	require.NoError(t, err)
	require.Equal(t, expectedRewardAmt, rewards.Rewards.AmountOf(core.MicroDoDenom).TruncateInt())
...
func TestOracleMultiRewardDistribution(t *testing.T) {
	...
	// SDR and KRW have the same voting power, but KRW has been chosen as referenceDo by alphabetical order.
	// Account 1, SDR, KRW
	...
	err := input.BankKeeper.MintCoins(input.Ctx, types.ModuleName, sdk.NewCoins(sdk.NewCoin(core.MicroDoDenom, rewardAmt)))
	require.NoError(t, err)
	...
	expectedRewardAmt2 := sdkmath.ZeroInt() // even vote power is same KRW with SDR, KRW chosen referenceDo because alphabetical order
	...
	rewards, err = input.DistrKeeper.GetValidatorOutstandingRewards(input.Ctx.WithBlockHeight(2), keeper.ValAddrs[1])
	require.NoError(t, err)
	require.Equal(t, expectedRewardAmt2, rewards.Rewards.AmountOf(core.MicroDoDenom).TruncateInt())
	rewards, err = input.DistrKeeper.GetValidatorOutstandingRewards(input.Ctx.WithBlockHeight(2), keeper.ValAddrs[2])
	require.NoError(t, err)
	require.Equal(t, expectedRewardAmt3, rewards.Rewards.AmountOf(core.MicroDoDenom).TruncateInt())
}
...
func TestOracleExchangeRate(t *testing.T) {
	...
	// KRW has been chosen as referenceDo by highest voting power
	// Account 1, USD, KRW
...
func TestOracleEnsureSorted(t *testing.T) {
	...
	for i := 0; i < 100; i++ {
		krwDoExchangeRate1 := sdkmath.LegacyNewDecWithPrec(int64(rand.Uint64()%100000000), 6).MulInt64(core.MicroUnit)
		usdExchangeRate1 := sdkmath.LegacyNewDecWithPrec(int64(rand.Uint64()%100000000), 6).MulInt64(core.MicroUnit)

		krwDoExchangeRate2 := sdkmath.LegacyNewDecWithPrec(int64(rand.Uint64()%100000000), 6).MulInt64(core.MicroUnit)
		usdExchangeRate2 := sdkmath.LegacyNewDecWithPrec(int64(rand.Uint64()%100000000), 6).MulInt64(core.MicroUnit)

		krwDoExchangeRate3 := sdkmath.LegacyNewDecWithPrec(int64(rand.Uint64()%100000000), 6).MulInt64(core.MicroUnit)
		usdExchangeRate3 := sdkmath.LegacyNewDecWithPrec(int64(rand.Uint64()%100000000), 6).MulInt64(core.MicroUnit)

		// Account 1, USD, KRW
		makeAggregatePrevoteAndVote(t, input, h, 0, sdk.DecCoins{{Denom: core.MicroUSDDenom, Amount: usdExchangeRate1}, {Denom: core.MicroKRWDenom, Amount: krwDoExchangeRate1}}, 0)

		// Account 2, USD, KRW
		makeAggregatePrevoteAndVote(t, input, h, 0, sdk.DecCoins{{Denom: core.MicroUSDDenom, Amount: usdExchangeRate2}, {Denom: core.MicroKRWDenom, Amount: krwDoExchangeRate2}}, 1)

		// Account 3, USD, KRW
		makeAggregatePrevoteAndVote(t, input, h, 0, sdk.DecCoins{{Denom: core.MicroUSDDenom, Amount: usdExchangeRate3}, {Denom: core.MicroKRWDenom, Amount: krwDoExchangeRate3}}, 2)
...
func TestOracleExchangeRateVal5(t *testing.T) {
	...
	krwDoExchangeRate := sdkmath.LegacyNewDecWithPrec(505000, int64(6)).MulInt64(core.MicroUnit)
	krwDoExchangeRateWithErr := sdkmath.LegacyNewDecWithPrec(500000, int64(6)).MulInt64(core.MicroUnit)
	...
	// KRW has been chosen as referenceDo by highest voting power
	// Account 1, KRW, USD
	makeAggregatePrevoteAndVote(t, input, h, 0, sdk.DecCoins{{Denom: core.MicroKRWDenom, Amount: krwDoExchangeRate}, {Denom: core.MicroUSDDenom, Amount: usdExchangeRate}}, 0)

	// Account 2, KRW
	makeAggregatePrevoteAndVote(t, input, h, 0, sdk.DecCoins{{Denom: core.MicroKRWDenom, Amount: krwDoExchangeRate}}, 1)

	// Account 3, KRW
	makeAggregatePrevoteAndVote(t, input, h, 0, sdk.DecCoins{{Denom: core.MicroKRWDenom, Amount: krwDoExchangeRate}}, 2)

	// Account 4, KRW, USD
	makeAggregatePrevoteAndVote(t, input, h, 0, sdk.DecCoins{{Denom: core.MicroKRWDenom, Amount: krwDoExchangeRateWithErr}, {Denom: core.MicroUSDDenom, Amount: usdExchangeRateWithErr}}, 3)

	// Account 5, KRW, USD
	makeAggregatePrevoteAndVote(t, input, h, 0, sdk.DecCoins{{Denom: core.MicroKRWDenom, Amount: krwDoExchangeRateWithErr}, {Denom: core.MicroUSDDenom, Amount: usdExchangeRateWithErr}}, 4)
	...
	krw, err := input.OracleKeeper.GetDoExchangeRate(input.Ctx, core.MicroKRWDenom)
	require.NoError(t, err)
	usd, err := input.OracleKeeper.GetDoExchangeRate(input.Ctx, core.MicroUSDDenom)
	require.NoError(t, err)
	...
	// new version case
	require.Equal(t, krwDoExchangeRate, krw)
	...
	require.NoError(t, err)
	require.Equal(t, expectedRewardAmt2, rewards1.Rewards.AmountOf(core.MicroDoDenom).TruncateInt())
	rewards2, err := input.DistrKeeper.GetValidatorOutstandingRewards(input.Ctx.WithBlockHeight(2), keeper.ValAddrs[2])
	require.NoError(t, err)
	require.Equal(t, expectedRewardAmt2, rewards2.Rewards.AmountOf(core.MicroDoDenom).TruncateInt())
	rewards3, err := input.DistrKeeper.GetValidatorOutstandingRewards(input.Ctx.WithBlockHeight(2), keeper.ValAddrs[3])
	require.NoError(t, err)
	require.Equal(t, expectedRewardAmt, rewards3.Rewards.AmountOf(core.MicroDoDenom).TruncateInt())
	rewards4, err := input.DistrKeeper.GetValidatorOutstandingRewards(input.Ctx.WithBlockHeight(2), keeper.ValAddrs[4])
	require.NoError(t, err)
	require.Equal(t, expectedRewardAmt, rewards4.Rewards.AmountOf(core.MicroDoDenom).TruncateInt())
}
...
func makeAggregatePrevoteAndVote(t *testing.T, input keeper.TestInput, h types.MsgServer, height int64, rates sdk.DecCoins, idx int) {
	...
	prevoteMsg := types.NewMsgAggregateDoRatePrevote(hash, keeper.Addrs[idx], keeper.ValAddrs[idx])
	_, err := h.AggregateDoRatePrevote(input.Ctx.WithBlockHeight(height), prevoteMsg)
	require.NoError(t, err)

	voteMsg := types.NewMsgAggregateDoRateVote(salt, rates.String(), keeper.Addrs[idx], keeper.ValAddrs[idx])

...
func TestFeeRewardsForEpoch(t *testing.T) {
	...
	// Set random prices
	input.OracleKeeper.SetDoExchangeRate(input.Ctx, core.MicroSDRDenom, sdkmath.LegacyNewDec(1))
	input.OracleKeeper.SetDoExchangeRate(input.Ctx, core.MicroKRWDenom, sdkmath.LegacyNewDec(10))
	input.OracleKeeper.SetDoExchangeRate(input.Ctx, core.MicroGBPDenom, sdkmath.LegacyNewDec(100))
	input.OracleKeeper.SetDoExchangeRate(input.Ctx, core.MicroCNYDenom, sdkmath.LegacyNewDec(1000))
...
func TestSeigniorageRewardsForEpoch(t *testing.T) {
	...
	// Add seigniorage
	err := input.BankKeeper.BurnCoins(input.Ctx, faucetAccountName, sdk.NewCoins(sdk.NewCoin(core.MicroDoDenom, amt)))

...
func TestQueryTaxCaps(t *testing.T) {
	...
	input.TreasuryKeeper.SetTaxCap(input.Ctx, "udotest", sdkmath.NewInt(1200000))
...
func TestQueryTaxProceeds(t *testing.T) {
	...
	taxProceeds := sdk.NewCoins(sdk.NewCoin(core.MicroDoDenom, sdkmath.NewInt(100)), sdk.NewCoin(core.MicroKRWDenom, sdkmath.NewInt(100)))
...
func TestQuerySeigniorageProceeds(t *testing.T) {
	...
	err := input.BankKeeper.BurnCoins(input.Ctx, faucetAccountName, sdk.NewCoins(sdk.NewCoin(core.MicroDoDenom, targetSeigniorage)))
	require.NoError(t, err)

...
var (
	...
	InitCoins  = sdk.NewCoins(sdk.NewCoin(core.MicroDoDenom, InitTokens))
)
...
func CreateTestInput(t *testing.T) TestInput {
	...
	totalSupply := sdk.NewCoins(sdk.NewCoin(core.MicroDoDenom, InitTokens.MulRaw(int64(len(Addrs)*10))))
	...
	stakingParams.BondDenom = core.MicroDoDenom
	...
	require.NoError(t, bankKeeper.SendCoinsFromModuleToModule(ctx, faucetAccountName, stakingtypes.NotBondedPoolName, sdk.NewCoins(sdk.NewCoin(core.MicroDoDenom, InitTokens.MulRaw(int64(len(Addrs)+1))))))
...
func NewTestMsgCreateValidator(address sdk.ValAddress, pubKey cryptotypes.PubKey, amt sdkmath.Int) *stakingtypes.MsgCreateValidator {
	...
	msg, _ := stakingtypes.NewMsgCreateValidator(
		sdk.AccAddress(address).String(), pubKey, sdk.NewCoin(core.MicroDoDenom, amt),

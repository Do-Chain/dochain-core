...
func SetupApp(t *testing.T, chainID string) *app.DoApp {
	t.Helper()
...
func SetupWithGenesisValSet(
	...
	doApp, genesisState := setup(chainID)
	genesisState = genesisStateWithValSet(t, doApp, genesisState, valSet, genAccs, balances...)

	stateBytes, err := json.MarshalIndent(genesisState, "", "")
	require.NoError(t, err)

	// InitChain writes all module genesis
	doApp.InitChain(&abci.RequestInitChain{
		ChainId:         chainID,
...
func setup(chainID string) (*app.DoApp, app.GenesisState) {
	db := dbm.NewMemDB()
	...
	baseDir, err := os.MkdirTemp("", "doapp")
	if err != nil {
		panic(err)
	}
	...
	return doapp, app.GenesisState{}
}

func genesisStateWithValSet(t *testing.T,
	app *app.DoApp, genesisState app.GenesisState,
	...
	bankGenesis := banktypes.NewGenesisState(
		...
		[]banktypes.SendEnabled{
			{Denom: appparams.BondDenom, Enabled: true}, // Enable udotest transfers
		},
	)
